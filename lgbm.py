...
# param_
    parameters = {
        ...
        'lambda_l1':[0.1,0.5,1,1.2],
        'lambda_l2':[0.1,0.5,1,1.2]
        }
    ...
    lgbm_cv_10_fold_result =pd.DataFrame( [cv_10_fold_auc, cv_10_fold_acc, cv_10_fold_bacc, cv_10_fold_recall, cv_10_fold_sp,
    ...
    lgbm_cv_10_fold_result_all.append(lgbm_cv_10_fold_result)
    lgbm_cv = pd.DataFrame([cv_result], index=[index1], columns=columns)
    lgbm_test = pd.DataFrame([test_result], index=[index2], columns=columns)
    result_cv.append(lgbm_cv)
    result_test.append(lgbm_test)
...
result_all_10_fold_result = pd.concat(lgbm_cv_10_fold_result_all,axis=0)
result_all_test_pred = pd.concat(test_pred_all,axis=1)
result_all_test_prob  =pd.concat(test_prob_all,axis=1)
result_all_cv.to_csv("lgbm_result/" + 'lgbm_result_cv.csv')
result_all_test.to_csv("lgbm_result/" + 'lgbm_result_test.csv')
result_all_param.to_csv("lgbm_result/" + 'lgbm_result_param.csv')
result_all_10_fold_result.to_csv("lgbm_result/"+'lgbm_cv_10_fold_result.csv')
result_all_test_pred.to_csv("lgbm_result/" + 'lgbm_result_test_pred.csv',index=None)
result_all_test_prob.to_csv("lgbm_result/" + 'lgbm_result_test_prob.csv',index=None)
